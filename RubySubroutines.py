            ...
            match prim.measurement_within_tol(
                ...
                case True:
                    ...
                    if child is not None:
                        child.setText(
                            f"{key} ({measured[key]:.3f}) within tol: {targetMeasurement:.2f} +/- {measurementTolerance:.2f}"
                        )
                        child.setStyleSheet("background-color: lightgreen")
                case False:
                    child = self.window.findChild(QLabel, label)
                    if child is not None:
                        text = f"{key} ({measured[key]:.3f}) outside tol: {targetMeasurement:.2f} +/- {measurementTolerance:.2f}"
                        child.setText(text)
